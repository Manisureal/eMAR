    ...
    if (admin.dose_given === admin.dose_prescribed || admin.false_reason != null || admin.dose_prescribed === doseGivenSum.toString() || doseGivenSum === false) {
      ...
      if (tickCrossDoseSmilyFlag === false) {
        ...
        if (item.available_quantity === 0){
          ...
          // patientInfo+="<div id='administer-"+itemId+"'>"+"<button onclick='medicationAdministration("+itemId+", \""+slotTime+"\")'>"+"<i class='fas fa-check'></i>"+"</button>"+"</div>"
          if (item.is_insulin){
            patientInfo+="<i style='padding-right:15px;' onclick='medWasGivenLaterThanPreviousSlotTime("+itemId+",\""+slotTime+"\")' class='fas fa-check fa-lg' id='item-"+itemId+"-"+slotTime.replace(":", "")+"'></i>"
            // patientInfo+="<i style='padding-right:15px;' onclick='medAlreadySelected("+itemId+",\""+slotTime+"\")' class='fas fa-check fa-lg' id='item-"+itemId+"-"+slotTime.replace(":", "")+"'></i>"
